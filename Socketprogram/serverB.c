...
void fileread()
{
	...
	strcat(msg,"serverB");
	//strcat(msg,"END");
    printf("The server B has following neighbour information \nNeighbour ----- cost\n");
...
void printadjacancymatrix()
{
 ...
 a[2] = 'C';
 a[3] = 'D';
 printf("The adjacancy Matrix for server B is \n");
 printf("\nserver\tA\tB\tC\tD\n");
 for(i=0;i<4;i++)
 {
	 for(j =0 ;j <4; j++)
...
printf("For this connection with Client, The Server B has %s UDP port number and %s IP address\n",SERVERBUDPPORT,Ipadressforthisserver);
