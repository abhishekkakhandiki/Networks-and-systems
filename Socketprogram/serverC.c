...
void  fileread()
{
	...
	printf("The ServerC is up and running\n");
	strcat(msg,"serverC");
	//strcat(msg,"END");
    printf("The server C has following neighbour information \nNeighbour ----- cost\n");
...
void printadjacancymatrix()
{
 ...
 a[2] = 'C';
 a[3] = 'D';
 printf("The adjacancy Matrix for server C is \n");
 printf("\nserver\tA\tB\tC\tD\n");
 for(i=0;i<4;i++)
 {
	 for(j =0 ;j <4; j++)
...
printf("For this connection with Client, The Server C has %s UDP port number and %s IP address\n",SERVERCUDPPORT,Ipadressforthisserver);
