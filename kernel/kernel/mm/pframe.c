...
pframe_get(struct mmobj *o, uint32_t pagenum, pframe_t **result)
{
	...
	/*  NOT_YET_IMPLEMENTED("S5FS: pframe_get");*/
		...
		*result=pframe_get_resident(o,pagenum);
		dbg(DBG_PRINT,"In p Frame get\n");
		if(*result!=NULL){
			while((*result)->pf_flags == PF_BUSY){
				dbg(DBG_PRINT,"In p Frame while\n");
...
{
	if(pframe_lookup(pf->pf_obj,pf->pf_pagenum,0,&pf) == 0){
			dbg(DBG_PRINT,"In p Frame pin\n");
			if(!pframe_is_pinned(pf)){
				list_remove(&pf->pf_link);
				nallocated--;
...
{
	if(--pf->pf_pincount  == 0){
			dbg(DBG_PRINT,"In p Frame unpin\n");
			list_remove(&pf->pf_link);
			nallocated++;
			list_insert_tail(&alloc_list,&pf->pf_link);
			npinned--;
