...
static vnode_ops_t blockdev_spec_vops = { .read = NULL, .write = NULL, .mmap =
		NULL, .create = NULL, .mknod = NULL, .lookup = NULL, .link = NULL,
		.unlink = NULL, .mkdir = NULL, .rmdir = NULL, .readdir = NULL, .stat =
				special_file_stat, .fillpage = NULL, .dirtypage = NULL,
				.cleanpage = NULL };
...
static int special_file_read(vnode_t *file, off_t offset, void *buf,
	...
	dbg(DBG_PRINT,"GRADING 2B in special_file_read\n");
	devid_t dev_id = 0;
	bytedev_t *dev;
	int bytes_read = 0;
	KASSERT(file);
	...
	dbg(DBG_PRINT, "GRADING2A 1.a Mode is either of S_ISCHR and S_ISBLK\n");
	if (S_ISCHR(file->vn_mode))
		KASSERT(file->vn_cdev && file->vn_cdev->cd_ops&& file->vn_cdev->cd_ops->read);
	dbg(DBG_PRINT, "GRADING2A 1.a Read for S_ISCHR\n");
	dev_id = file->vn_devid;

	if (dev_id == file->vn_cdev->cd_id) {
		dbg(DBG_PRINT,"GRADING 2B in special_file_read\n");
		dev = file->vn_cdev;
		bytes_read = dev->cd_ops->read(dev, offset, buf, count);
...
static int special_file_write(vnode_t *file, off_t offset, const void *buf,
		size_t count) {
	dbg(DBG_PRINT,"GRADING 2B in special_file_write\n");
	devid_t dev_id = 0;
	bytedev_t *dev;
	int bytes_write = 0;
	KASSERT(file);
	...
	KASSERT(S_ISCHR(file->vn_mode)||S_ISBLK(file->vn_mode));
	dbg(DBG_PRINT, "GRADING2A 1.b Mode is either of S_ISCHR and S_ISBLK\n");
	if (S_ISCHR(file->vn_mode))
		KASSERT(file->vn_cdev && file->vn_cdev->cd_ops
				&& file->vn_cdev->cd_ops->write);
	dbg(DBG_PRINT, "GRADING2A 1.b Write for S_ISCHR\n");
	dev_id = file->vn_devid;
	if (dev_id == file->vn_cdev->cd_id) {
		dbg(DBG_PRINT,"GRADING 2B in special_file_write\n");
		dev = file->vn_cdev;
		bytes_write = dev->cd_ops->write(dev, offset, buf, count);
	}
	return bytes_write;
}
