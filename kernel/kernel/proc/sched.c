...
void sched_sleep_on(ktqueue_t *q) {
	...
	sched_switch();
	/*while(curthr->kt_state==KT_SLEEP);*//*To do?*/
}
...
sched_wakeup_on(ktqueue_t *q) {
	...
	dbg(DBG_PRINT,
			"(GRADING1A 4.a) thr is the pointer to the corresponding thread\n");
...
void sched_switch(void) {
	...
	/*How to restore old context ??*/
	oldcontext = &curthr->kt_ctx;
	intr_setipl(IPL_HIGH);
	/*ktqueue_remove(&kt_runq,curthr);*/
	kthread_t *runnablethread = ktqueue_dequeue(&kt_runq);
	curproc = runnablethread->kt_proc;
	curthr = runnablethread;

	/* switching context from old thread to new thread */
	context_switch(oldcontext, &runnablethread->kt_ctx);
...
void sched_make_runnable(kthread_t *thr) {
	...
	dbg(DBG_PRINT, "(GRADING1A 4.b) Thread is not in the run Queue\n");
